...
try {
    ...
    error_log("[ENROLL] JSON decodificado OK para number_id: " . ($input['number_id'] ?? 'N/A'));
    ...
    error_log("[ENROLL] Campos validados OK");
    ...
    $stmtPeriod->close();
    error_log("[ENROLL] Periodo encontrado, start_date: " . $start_date);
    ...
    $stmtUser->close();
    error_log("[ENROLL] Usuario encontrado, creationDate: " . $creationDate);
    ...
    error_log("[ENROLL] Fechas validadas OK");
    ...
    $sql = "INSERT INTO groups (
        ...
        bootcamp_teacher_id, bootcamp_mentor_id, bootcamp_monitor_id,
        id_bootcamp, bootcamp_name, b_intensity, b_reals,
        le_teacher_id, le_mentor_id, le_monitor_id,
        id_leveling_english, leveling_english_name, le_intensity, le_reals,
        ec_teacher_id, ec_mentor_id, ec_monitor_id,
        id_english_code, english_code_name, ec_intensity, ec_reals,
        skills_teacher_id, skills_mentor_id, skills_monitor_id,
        id_skills, skills_name, s_intensity, s_reals, cohort
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    $stmt->bind_param(
        ...
        $input['program'],
        $input['mode'],
        $zero, $zero, $zero,                               // bootcamp teacher/mentor/monitor
        $input['id_bootcamp'],
        $input['bootcamp_name'],
        $zero, $zero,                                       // b_intensity, b_reals
        $zero, $zero, $zero,                               // le teacher/mentor/monitor
        $input['id_leveling_english'],
        $input['leveling_english_name'],
        $zero, $zero,                                       // le_intensity, le_reals
        $zero, $zero, $zero,                               // ec teacher/mentor/monitor
        $input['id_english_code'],
        $input['english_code_name'],
        $zero, $zero,                                       // ec_intensity, ec_reals
        $zero, $zero, $zero,                               // skills teacher/mentor/monitor
        $input['id_skills'],
        $input['skills_name'],
        $zero, $zero,                                       // s_intensity, s_reals
        $cohort_number
    );
    ...
    error_log("[ENROLL] INSERT en groups OK para number_id: " . $input['number_id']);

    // Crear usuario en Moodle
    $moodleAPI = new MoodleAPI();
    $nombres = array_slice(explode(' ', $input['full_name']), 0, 2);
    $apellidos = array_slice(explode(' ', $input['full_name']), 2);

    $moodleUserId = null;

    // Verificar si el usuario ya existe en Moodle
    $existingUser = $moodleAPI->getUserByUsername($input['number_id']);
    if ($existingUser && isset($existingUser['id'])) {
        $moodleUserId = $existingUser['id'];
        error_log("[ENROLL] Usuario ya existe en Moodle, ID: " . $moodleUserId);
    } else {
        // Crear nuevo usuario con contraseña que cumpla política de Moodle
        $password = 'Cendi@2026';
        $moodleUser = [
            'username' => $input['number_id'],
            'password' => $password,
            'firstname' => implode(' ', $nombres),
            'lastname' => implode(' ', $apellidos),
            'email' => $input['institutional_email']
        ];

        $createResponse = $moodleAPI->createUser($moodleUser);
        error_log("[ENROLL] Respuesta Moodle createUser: " . json_encode($createResponse));

        if (!isset($createResponse[0]['id'])) {
            $apiError = $createResponse['message'] ?? ($createResponse['error'] ?? json_encode($createResponse));
            throw new Exception("Error creando usuario en Moodle: " . $apiError);
        }
    ...
    error_log("[ENROLL] ERROR: " . $e->getMessage() . " | Trace: " . $e->getTraceAsString());
