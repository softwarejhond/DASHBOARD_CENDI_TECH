...
<script>
    ...
    let usersDataGlobal = null;
    let dataLoaded = false;

    function loadRegisterCourseData() {
        if (dataLoaded) return Promise.resolve();
        return Promise.all([
            fetch('components/modals/get_courses.php').then(r => r.json()),
            fetch('components/modals/get_users.php').then(r => r.json())
        ]).then(([courses, users]) => {
            coursesDataGlobal = Array.isArray(courses) ? courses : (courses.courses || courses.data || []);
            console.log('Courses loaded:', coursesDataGlobal.length, 'courses');
            usersDataGlobal = users;
            populateUserSelects();
            dataLoaded = true;

            if ($('#typeCourse').val()) {
                $('#typeCourse').trigger('change');
            }
        }).catch(err => {
    ...
    function populateUserSelects() {
        if (!usersDataGlobal) return;
        const users = usersDataGlobal;
        let teacherHtml = '<option value="">Seleccione un profesor</option>';
        let mentorHtml = '<option value="">Seleccione un mentor</option>';
        let monitorHtml = '<option value="">Seleccione un monitor</option>';
        users.forEach(function(user) {
            if (user.rol == 5) {
                teacherHtml += '<option value="' + user.username + '">' + user.nombre + '</option>';
            }
            if (user.rol == 8 || user.rol_informativo == 8) {
                mentorHtml += '<option value="' + user.username + '">' + user.nombre + '</option>';
            }
            if (user.rol == 7 || user.rol_informativo == 7) {
                monitorHtml += '<option value="' + user.username + '">' + user.nombre + '</option>';
            }
        });
        $('#teacherSelect').html(teacherHtml);
        $('#mentorSelect').html(mentorHtml);
        $('#monitorSelect').html(monitorHtml);

        updateDropdownOptions($('#teacherSelect'), $('#teacherSelect').parent().find('.custom-select-dropdown'), '');
        updateDropdownOptions($('#mentorSelect'), $('#mentorSelect').parent().find('.custom-select-dropdown'), '');
        updateDropdownOptions($('#monitorSelect'), $('#monitorSelect').parent().find('.custom-select-dropdown'), '');
    }

    $('#registerCourseModal').on('show.bs.modal', function() {
        loadRegisterCourseData();
    });
    ...
    $(document).ready(function() {
        ...
        // Manejador del cambio en tipo de curso
        $('#typeCourse').change(async function() {
            let type = $(this).val();

            if (!type) {
                $('#courseSelect').prop('disabled', true);
                let displayField = $('#courseSelect').parent().find('.custom-select-field');
                if (displayField.length) displayField.text('Seleccione un curso');
                return;
            }
            ...
            let displayField = $('#courseSelect').parent().find('.custom-select-field');
            if (displayField.length) displayField.text('Cargando cursos...');

            await loadRegisterCourseData();

            // Determinar categorías según el tipo
            let categories = [];
            switch (type) {
                case 'tecnico':
                    categories = [3, 7, 8, 9, 10, 11, 12];
                    break;
                case 'nivelatorio':
                    categories = [4];
                    break;
            ...
            if (coursesDataGlobal && coursesDataGlobal.length) {
                let found = 0;
                coursesDataGlobal.forEach(course => {
                ...
                console.log('Filtered courses for type "' + type + '" (categories ' + JSON.stringify(categories) + '): ' + found + ' found');
            } else {
                console.error('No courses data available after loading');
            }

            // Reconstruir el dropdown del custom select
    ...
    function updateDropdownOptions(select, dropdown, searchText) {
        dropdown.find('.custom-select-option').remove();
        select.find('option').each(function() {
            const option = $(this);
            const text = option.text();
