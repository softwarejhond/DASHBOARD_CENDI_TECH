<?php
// =====================================================================
// Endpoint server-side para el listado de matriculados (DataTables).
// Devuelve JSON con paginación, búsqueda y ordenamiento en lotes.
// Tabla principal: enrollments (última matrícula por number_id),
// con LEFT JOIN a cursos para el nombre del curso técnico.
// Filtros adicionales: set_id (set de cursos) y number_id (cédula).
// =====================================================================
...
while ($row = $result->fetch_assoc()) {
    $data[] = [
        ...
        'created_at'            => fmtDateEnrolled($row['created_at']),
    ];
}
$stmt->close();
$conn->close();
