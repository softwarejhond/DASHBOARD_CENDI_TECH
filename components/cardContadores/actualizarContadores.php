...
try {
    ...
    $sql_matriculados_grupos = "SELECT COUNT(DISTINCT g.number_id) as total_matriculados_grupos 
                                 FROM groups g 
                                 INNER JOIN user_register ur ON g.number_id = ur.number_id";
    $result_matriculados_grupos = mysqli_query($conn, $sql_matriculados_grupos);
    $total_matriculados_grupos = mysqli_fetch_assoc($result_matriculados_grupos)['total_matriculados_grupos'];

    $querySedes = "SELECT headquarters, COUNT(*) as cantidad FROM user_register GROUP BY headquarters";
    $resultadoSedes = $conn->query($querySedes);
    $sedes = [];
    ...
    $querySedesMatriculados = "
        SELECT ur.headquarters, COUNT(DISTINCT g.number_id) as cantidad
        FROM groups g
        INNER JOIN user_register ur ON g.number_id = ur.number_id
        GROUP BY ur.headquarters
    ";
    $resultadoSedesMatriculados = $conn->query($querySedesMatriculados);
    $sedesMatriculados = [];
    while ($row = $resultadoSedesMatriculados->fetch_assoc()) {
        $sedesMatriculados[] = [
            'sede' => $row['headquarters'],
            'cantidad' => $row['cantidad']
        ];
    ...
    $queryBootcamps = "
        ...
        FROM groups g
        INNER JOIN user_register ur ON g.number_id = ur.number_id
        WHERE g.bootcamp_name IS NOT NULL AND g.bootcamp_name != ''
        GROUP BY g.bootcamp_name
        ORDER BY cantidad DESC
    ";
    $resultadoBootcamps = $conn->query($queryBootcamps);
    $bootcamps = [];
    while ($row = $resultadoBootcamps->fetch_assoc()) {
        $bootcamps[] = [
    ...
    echo json_encode([
        ...
        "generos" => $generos,
        "total_matriculados_grupos" => $total_matriculados_grupos,
        "sedes" => $sedes,
        "sedesMatriculados" => $sedesMatriculados,
        "bootcamps" => $bootcamps
    ]);
} catch (Exception $e) {
