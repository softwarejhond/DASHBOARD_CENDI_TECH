<?php
// =====================================================================
// Endpoint AJAX para el selector de "set de curso" (select2) del módulo
// de diplomas. Solo devuelve los sets que tienen estudiantes en
// enrollments (no todos), con búsqueda y paginación.
// Tablas: sets_cursos + cursos + enrollments.
// =====================================================================
...
$baseFrom = "FROM sets_cursos sc
    LEFT JOIN cursos c ON sc.curso_tecnico_id = c.course_id
    WHERE EXISTS (SELECT 1 FROM enrollments e WHERE e.set_id = sc.id)";

$params = [];
$types  = '';
if ($q !== '') {
    $like   = '%' . $q . '%';
    $baseFrom .= " AND (sc.codigo_tecnico LIKE ? OR sc.serie LIKE ? OR c.course_name LIKE ?)";
